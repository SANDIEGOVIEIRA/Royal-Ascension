...
if (abs(dx) > abs(dy)) {
    ...
    if (dx > 0) {
        sprite_index = spr_clone_direita_a; // Quando o personagem está à direita
    } else {
        sprite_index = spr_clone_esquerda_a; // Quando o personagem está à esquerda
    }
} else {
    // Movimento mais vertical
    if (dy > 0) {
        sprite_index = spr_clone_a; // Quando o personagem está abaixo
    } else {
        sprite_index = spr_clone_cima_a; // Quando o personagem está acima
    }
}
 ...
 if (alarm[3] <= 0) {
        ...
        alarm[3] = 190; // Tempo entre os disparos
    }
