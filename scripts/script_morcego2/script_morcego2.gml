...
function scr_checar_personagem_morcego2(){
	if distance_to_object(obj_personagem) <= dist_atk{
	estado = scr_morcego2_perseguindo;
	}
}


function scr_morcego2_escolher_estado(){
	scr_checar_personagem_morcego2();
	prox_estado = choose(scr_morcego2_andando, scr_morcego2_parado);
	
	if prox_estado == scr_morcego2_andando{
	estado = scr_morcego2_andando;
	dest_x = irandom_range(0, room_width);
	dest_y = irandom_range(0, room_height);
	
	}else if prox_estado == scr_morcego2_parado{
	estado = scr_morcego2_parado;
	
	}
}

function scr_morcego2_andando(){
    scr_checar_personagem_morcego2();
    image_speed = 1;
    
    // Pare a reprodução de áudio quando o inimigo não estiver mais perseguindo o personagem
    if (audio_playing1) {
        audio_stop_sound(snd_asas);
...
function scr_morcego2_perseguindo(){
    image_speed = 1.5;
    ...
    if (!audio_playing1) {
        audio_play_sound(snd_asas, 0, false);
		audio_stop_sound(rastejar);
        audio_playing1 = true;
    }
    
    if distance_to_object(obj_personagem) >= dist_no_atk{
        estado = scr_morcego2_escolher_estado;
    }
}

function scr_morcego2_hit(){
	alarm[2] = 180;
